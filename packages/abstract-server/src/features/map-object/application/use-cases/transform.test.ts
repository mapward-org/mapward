...
import { createBuiltins } from "../services/builtin.ts";
...
test("встроенный шаг зовётся по имени и метит узлы", async () => {
  ...
  const result = await transform(ports, createBuiltins(ports), metric, owner, "/map", collected);
...
test("кэш трансформа пишется как обычно — пометки в нём и лежат", async () => {
  ...
  await transform(ports, createBuiltins(ports), metric, owner, "/map", collected);
