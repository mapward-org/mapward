...
export function directivePrompt(
  ...
  const head = [
    ...
    "Бери контекст разом, а не по объекту за вызов: depth раскрывает детей такими же",
    "объектами, fields оставляет в ответе только нужные поля. Например так:",
    'read_object { depth: 2, refresh: "on-display", fields: ["address", "name", "props",',
    '  "metrics.key", "metrics.label", "metrics.value", "directives", "actions",',
    '  "children.address", "children.name"] }.',
    "Без fields приходит и конфиг каждой метрики — он нужен, только когда метрику правят.",
    "Соседние объекты берутся из того же ответа: parents ведут вверх, children вниз.",
