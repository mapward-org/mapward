...
import { fileLinks, markGit } from "../../domain/git-mark.ts";
...
export class Builtins {
  private readonly steps: Record<string, BuiltinStep>;
  ...
  /** Есть ли такой встроенный шаг: по нему трансформ решает, звать реестр или оболочку. */
  step(kind: string): BuiltinStep | undefined {
    return this.steps[kind];
  }

  /** Встроенные шаги метрики по порядку — их вотчеры поднимает стор. */
  stepsOf(config: MetricConfig): BuiltinStep[] {
    return (config.transforms ?? [])
      .map((spec) => this.steps[kindOf(spec)])
   ...
   */
  stateful(config: MetricConfig): boolean {
    return this.stepsOf(config).length > 0;
  }
}
