...
import type { ActionInput, MapAction, MapObject } from "@mapward/core";
...
/** Поле формы запуска готовым: чем его рисовать и что в нём сейчас. */
export type FormField = {
  name: string;
  id: string;
  kind: "boolean" | "choice" | "number" | "multiline" | "text";
  text: string;
  checked: boolean;
  options: string[];
  required: boolean;
...
const kindOf = (input: ActionInput): FormField["kind"] => {
  const type = input.type ?? "string";
  if (type === "boolean" || type === "choice" || type === "number") return type;
  return input.multiline ? "multiline" : "text";
};
...
export function formFields(
  ...
  return Object.entries(inputs).map(([name, input], index) => {
